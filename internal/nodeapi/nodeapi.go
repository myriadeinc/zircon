package nodeapi

import (
	"bytes"
	"encoding/json"
	"fmt"
	"io"
	"net/http"
	"strconv"

	"github.com/myriadeinc/zircon/internal/models"
	"github.com/rs/zerolog/log"
	"github.com/spf13/viper"
)

type NodeApi interface {
	GetValidBlockTemplate() (*models.StrictTemplate, error)
}

type NodeClient struct {
	nodes []string
}

func NewNodeClient(nodes []string) NodeApi {
	client := NodeClient{
		nodes: nodes,
	}
	return &client
}

// Block template has these fields
// {

// 	"blockhashing_blob": "0e0e8c89eb9606b44e67bdbc91e5f95c5ea49ba410b1c223ab897417b19a0f631b80cf5be43a4e00000000f608c45071e983a7fbcd9d1474bfa1505d4d808b2bfcd513f214a3820c41942d17",
// 	"blocktemplate_blob": "",
// 	"difficulty": 307042777507,
// 	"difficulty_top64": 0,
// 	"expected_reward": 600450790000,
// 	"height": 2672960,
// 	"next_seed_hash": "",
// 	"prev_hash": "b44e67bdbc91e5f95c5ea49ba410b1c223ab897417b19a0f631b80cf5be43a4e",
// 	"reserved_offset": 130,
// 	"seed_hash": "348b6ead39ba7ebb7ab45bbada3e8e7c3c171e257671e5ac570867c953d5bcf9",
// 	"seed_height": 2672640,
// 	"status": "OK",
// 	"untrusted": false,
// 	"wide_difficulty": "0x477d2cf9a3"

// }
func (n *NodeClient) GetValidBlockTemplate() (*models.StrictTemplate, error) {
	walletAddress := viper.GetString("WALLET_ADDRESS")
	reserveOffset := viper.GetInt("RESERVE_OFFSET")

	request, _ := json.Marshal(map[string]interface{}{
		"jsonrpc": "2.0",
		"id":      "2",
		"method":  "get_block_template",
		"params": struct {
			WalletAddress string `json:"wallet_address"`
			ReserveSize   string `json:"reserve_size"`
		}{
			WalletAddress: walletAddress,
			ReserveSize:   strconv.Itoa(reserveOffset),
		},
	})

	// log.Debug().Str("requestbody", string(request)).Msg("crafted payload")
	rawTemplate, err := n.fetchFromNodes(request)
	if err != nil {
		return nil, err
	}
	return models.NewStrictTemplate(rawTemplate)

}
func makeRequest(payload []byte, node string) ([]byte, error) {
	// log.Info().Str("url", node).Msg("making a request to node")
	resp, err := http.Post(node, "application/json", bytes.NewBuffer(payload))
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, err
	}

	return body, nil

}

func (n *NodeClient) fetchFromNodes(payload []byte) (map[string]interface{}, error) {
	// log.Debug().Interface("nodes", n.nodes).Msg("using node selection")
	for _, node := range n.nodes {
		body, err := makeRequest(payload, node)
		if err != nil {
			log.Error().Err(err).Str("url", node).Msg("could not use node")
			continue
		}
		template, err := convertResponseToTemplate(body, node)
		if err != nil {
			continue
		}
		// log.Info().Interface("template", template).Msg("tempalte")
		if len(template) > 0 {
			// log.Debug().Msg("got valid template")
			return template, nil
		}
	}

	return nil, fmt.Errorf("could not get any response from all nodes")
}

func convertResponseToTemplate(response []byte, node string) (map[string]interface{}, error) {
	holder := struct {
		Result map[string]interface{} `json:"result,omitempty"`
	}{}

	err := json.Unmarshal(response, &holder)
	if err != nil {
		log.Error().Str("response", string(response)).Str("node", node).Err(err).Msg("node bad response")
		return nil, err
	}
	// log.Debug().Str("result", string(response)).Msg("raw response")
	return holder.Result, nil

}
